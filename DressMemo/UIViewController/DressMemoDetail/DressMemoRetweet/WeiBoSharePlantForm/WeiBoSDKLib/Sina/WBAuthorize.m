...
- (NSString *)tmpTokenUsingAppKey
{
    ...
    NSString *str = [_request syncConnect];
    
    _request = nil;
    
    NSDictionary *params = [NSURL parseURLQueryString:str];
	NSString *tmpTokenKey = [params objectForKey:@"oauth_token"];
	NSString *tmpTokenSecret = [params objectForKey:@"oauth_token_secret"];
    
    self.requestToken = tmpTokenKey;
    self.requestSecret = tmpTokenSecret;
...
- (NSURLRequest *)authRequest:(NSString *)requestToken
{
    
	NSString* urlString = [WBRequest serializeURL:oauthUserAuthorizeURL 
										   params:[NSDictionary dictionaryWithObjectsAndKeys:requestToken,@"oauth_token",[NSString stringWithFormat:@"%@://%@",[_weibo urlSchemeString],WeiBoAuthorizeCallBack],@"oauth_callback",nil] 
									   httpMethod:@"GET"];
