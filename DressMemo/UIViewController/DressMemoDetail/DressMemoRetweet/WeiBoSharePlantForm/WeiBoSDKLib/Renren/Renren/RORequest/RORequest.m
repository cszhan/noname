...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error{
    ...
    SBJsonBase *jsonParser = [[SBJsonParser new] autorelease];
...
- (NSMutableData *)generatePostBody {
    ...
    if ([self isKindOfUIImage]) {
        ...
        id _dataParam=[_params valueForKey:@"upload"];
		NSData *imageData = nil;
		if([_dataParam isKindOfClass:[NSData class]])
		{
			imageData = _dataParam;
		}
		else if([_dataParam isKindOfClass:[UIImage class]]){
			
			imageData = UIImagePNGRepresentation((UIImage*)_dataParam);
		}
		else if([_dataParam isKindOfClass:[NSString class]]){
			imageData = [NSData dataWithContentsOfFile:_dataParam];
			if(imageData == nil){
				NSLog(@"pls check file path:%@",_dataParam);
			}
		}
		...
		NSLog(@"connection dict:%@",[self.params description]);
        for (NSString* key  in [_params keyEnumerator]) {
