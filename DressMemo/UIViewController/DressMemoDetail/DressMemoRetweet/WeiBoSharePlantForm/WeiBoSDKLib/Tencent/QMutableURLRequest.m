...
+ (NSMutableURLRequest *)requestPostWithFileData:(NSData *)fileData url:(NSString *)aUrl queryString:(NSString *)aQueryString{
    NSMutableURLRequest *request = [[[NSMutableURLRequest alloc] initWithURL:[NSURL smartURLForString:aUrl]] autorelease];
	[request setHTTPMethod:@"POST"];
	[request setTimeoutInterval:20.0f];
    ...
    NSString *header = [NSString stringWithFormat:headerTemplate, @"pic", [NSString stringWithFormat:@"%@%d", @"photoshare", count++]];
    [bodyData appendData:[header dataUsingEncoding:NSUTF8StringEncoding]];
    [bodyData appendData:fileData];
    [bodyData appendData:boundaryBytes];
